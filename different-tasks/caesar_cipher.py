#!/usr/bin/env python3
'''
Шифр Цезаря заключается в замене каждого символа входной строки на символ, находящийся на несколько позиций
левее или правее его в алфавите.
...
def decoder(shift):
    ...
    for i in input().strip():
        result += al[(al.find(i) + shift) % len(al)]
...
al = ' abcdefghijklmnopqrstuvwxyz'
shift = int(input())
res = ''.join(al[(al.index(i) + shift) % 27] for i in input().strip())
print(f'Result: "{res}"')
'''
